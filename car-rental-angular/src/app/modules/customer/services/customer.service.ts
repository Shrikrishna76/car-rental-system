...
export class CustomerService {
  ...
  // }
   ...
   searchCar(searchDto: any): Observable<any> {
      return this.http.post(`${BASIC_URL}/api/customer/car/search`, searchDto, {
        headers: this.createAuthorizationHeader()
      })
    }
  ...
  const token = StorageService.getToken(); // Make sure token exists
  if (token) {
...
cancelBookingById(bookingId: number) {
  return this.http.put(
    `${BASIC_URL}/api/customer/car/cancel/${bookingId}`,
    {},
    {
      headers: this.createAuthorizationHeader()  // ✅ Fix: Add this
    }
  );
}
